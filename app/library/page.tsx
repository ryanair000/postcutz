...
export default async function LibraryPage() {
  ...
  const [{ data: posters, error: posterError }, { data: unlocks }, { data: balance }] = await Promise.all([
    supabase.from(PORTAL.posters).select("*").eq("status", "published").order("featured", { ascending: false }).order("published_at", { ascending: false }),
